...
docker compose exec -T configSrv.1 mongosh --port 27017 <<EOF
rs.initiate(
  {
    _id : "config_server",
    configsvr: true,
...
rs.initiate(
    {
      _id : "shard1",
      members: [
        { _id : 0, host : "shard1.1:27020", priority: 2 },
...
rs.initiate(
    {
      _id : "shard2",
      members: [
        { _id : 0, host : "shard2.1:27023", priority: 2 },
        { _id : 1, host : "shard2.2:27024", priority: 1 },
        { _id : 2, host : "shard2.3:27025", priority: 1 }
      ]
    }
  );
exit();
